...
The goal for our CRM is to take these features as input and predict the direction of the train and also generate explanations for the prediction.

The first layer of the CRM are primitive features, like
...
Given a way to generate to the logical part of the CRM, the arithemtic part is generated using the same way as in a standard neural network using backpropagation, with each neuron ($n_i$) being _dropped out_ based on its predicate function ($f_i$).
